...
void calculateGridLayoutInternal(
  ...
  // Resolve style dimensions for every child before anything measures them, and
  // zero out display: none children on layout passes.
  for (auto child : node->getLayoutChildren()) {
    child->processDimensions();
    if (child->style().display() == Display::None && performLayout) {
  ...
  if (!widthIsDefinite || !heightIsDefinite) {
    ...
    if (!widthIsDefinite) {
      ...
      containerInnerWidth = boundAxis(
                                ...
                                FlexDirection::Row,
                                direction,
                                totalTrackWidth + paddingAndBorderInline,
                                ownerWidth,
                                ownerWidth) -
          paddingAndBorderInline;
      if (!yoga::inexactEquals(containerInnerWidth, totalTrackWidth)) {
    ...
    if (!heightIsDefinite) {
      ...
      containerInnerHeight = boundAxis(
                                 ...
                                 direction,
                                 totalTrackHeight + paddingAndBorderBlock,
                                 ownerHeight,
                                 ownerWidth) -
          paddingAndBorderBlock;
      if (!yoga::inexactEquals(containerInnerHeight, totalTrackHeight)) {
        containerSizeChanged = true;
      }
    }

    // The pass above sized the tracks against an indefinite container size,
    // where a percentage gap resolves to 0. Compare the gap it used with the gap
    // the resolved size gives: if they differ, those track sizes are stale.
    auto gapUsedForSizingIsStale = [&](Dimension dimension,
                                       float resolvedContainerSize) {
      return !yoga::inexactEquals(
          nodeStyle.computeGapForDimension(dimension, YGUndefined),
          nodeStyle.computeGapForDimension(dimension, resolvedContainerSize));
    };

    // We need to run track sizing again if:
    // 1. The container size changed due to min/max bounds or
    // 2. An indefinite dimension had percentage tracks, or a gap that resolves
    // differently now that the container size is known
    needsSecondTrackSizingPass = containerSizeChanged ||
        (!widthIsDefinite &&
         (trackSizing.hasPercentageTracks(Dimension::Width) ||
          gapUsedForSizingIsStale(Dimension::Width, containerInnerWidth))) ||
        (!heightIsDefinite &&
         (trackSizing.hasPercentageTracks(Dimension::Height) ||
          gapUsedForSizingIsStale(Dimension::Height, containerInnerHeight)));
  }
