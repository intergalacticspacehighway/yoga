...
// Only meaningful once initializeTrackSizes() has run for that dimension.
const TrackSizing::TrackFlags& TrackSizing::trackFlags(
    Dimension dimension) const {
  return dimension == Dimension::Width ? columnTrackFlags : rowTrackFlags;
}
...
TrackSizing::TrackSizing(
    ...
    BaselineItemGroups& baselineItemGroups): node(node),
...
void TrackSizing::runGridSizingAlgorithm() {
  ...
  if (columnTrackFlags.hasIntrinsicTracks) {
...
void TrackSizing::runTrackSizing(
    Dimension dimension,
...
// Also recomputes this dimension's TrackFlags for optimization purposes. This
// is the only writer; everything else reads them through trackFlags().
void TrackSizing::initializeTrackSizes(Dimension dimension) {
  ...
  TrackFlags& flags =
      dimension == Dimension::Width ? columnTrackFlags : rowTrackFlags;
  flags = TrackFlags{.hasOnlyFixedTracks = true};

  for (size_t i = 0; i < tracks.size(); i++) {
    auto& track = tracks[i];

    // detect percentage tracks for optimization purposes
    if (isPercentageSizingFunction(track.trackSize.minSizingFunction) ||
        isPercentageSizingFunction(track.trackSize.maxSizingFunction)) {
      flags.hasPercentageTracks = true;
    }

    if (isFixedSizingFunction(track.trackSize.minSizingFunction, containerSize)) {
      auto resolved = track.trackSize.minSizingFunction.resolve(containerSize);
      track.baseSize = resolved.unwrap();
    } else if (isIntrinsicSizingFunction(
                   track.trackSize.minSizingFunction, containerSize)) {
      track.baseSize = 0;
      flags.hasOnlyFixedTracks = false;
      flags.hasIntrinsicTracks = true;
    } else {
      // THIS SHOULD NEVER HAPPEN
      track.baseSize = 0;
    }

    if (isFixedSizingFunction(track.trackSize.maxSizingFunction, containerSize)) {
      auto resolved = track.trackSize.maxSizingFunction.resolve(containerSize);
      track.growthLimit = resolved.unwrap();
    } else if (isIntrinsicSizingFunction(
                   track.trackSize.maxSizingFunction, containerSize)) {
      track.growthLimit = INFINITY;
      flags.hasOnlyFixedTracks = false;
      flags.hasIntrinsicTracks = true;
    } else if (isFlexibleSizingFunction(track.trackSize.maxSizingFunction)) {
      track.growthLimit = INFINITY;
      flags.hasOnlyFixedTracks = false;
      flags.hasFlexibleTracks = true;
    } else {
      // THIS SHOULD NEVER HAPPEN
      track.growthLimit = INFINITY;
    }
...
void TrackSizing::resolveIntrinsicTrackSizes(Dimension dimension) {
  auto& tracks = dimension == Dimension::Width ? columnTracks : rowTracks;
  ...
  if (trackFlags(dimension).hasOnlyFixedTracks) {
    return;
  }
...
void TrackSizing::shimBaselineAlignedItems() {
...
void TrackSizing::accomodateSpanningItemsCrossingContentSizedTracks(
    Dimension dimension) {
  if (!trackFlags(dimension).hasIntrinsicTracks) {
    return;
  }
  auto& tracks = dimension == Dimension::Width ? columnTracks : rowTracks;
  auto containerSize = dimension == Dimension::Width ? containerInnerWidth
  ...
  for (size_t index = 0; index < gridItems.size(); index++) {
    ...
    if (span == 1) {
      ...
      // For auto minimums:
      if (isAutoSizingFunction(track.trackSize.minSizingFunction, containerSize)) {
      ...
      if (isAutoSizingFunction(track.trackSize.maxSizingFunction, containerSize)) {
        float contribution =
  ...
  for (auto& index : spanningItemIndices) {
    ...
    for (size_t i = startIndex; i < endIndex; i++) {
      if (isIntrinsicSizingFunction(
              tracks[i].trackSize.minSizingFunction, containerSize)) {
        intrinsicMinimumSizingFunctionTracks.push_back(&tracks[i]);
      }

      if (isIntrinsicSizingFunction(
              tracks[i].trackSize.maxSizingFunction, containerSize)) {
      ...
      if (isAutoSizingFunction(tracks[i].trackSize.maxSizingFunction, containerSize)) {
...
void TrackSizing::accomodateSpanningItemsCrossingFlexibleTracks(
    Dimension dimension) {
  if (!trackFlags(dimension).hasFlexibleTracks) {
    return;
  }
  auto& tracks = dimension == Dimension::Width ? columnTracks : rowTracks;
  auto sizingMode =
  ...
  for (const auto& item : gridItems) {
    ...
    for (size_t i = start; i < end && i < tracks.size(); i++) {
      auto& track = tracks[i];
      if (isFlexibleSizingFunction(track.trackSize.maxSizingFunction)) {
...
void TrackSizing::distributeExtraSpaceAcrossSpannedTracks(
    Dimension dimension,
...
void TrackSizing::distributeSpaceToFlexibleTracksForItems(
    Dimension dimension,
  ...
  for (const auto& itemSizeContribution : gridItemSizeContributions) {
    ...
    for (auto& track : itemSizeContribution.affectedTracks) {
      sumOfFlexFactors += track->trackSize.maxSizingFunction.value().unwrap();
    }

    if (sumOfFlexFactors > 0.0f) {
      // Distribute space by flex ratios (normalized)
      for (auto& track : itemSizeContribution.affectedTracks) {
        auto flexFactor = track->trackSize.maxSizingFunction.value().unwrap();
...
void TrackSizing::maximizeTrackSizes(Dimension dimension) {
  // Fast path - if tracks are fixed-sized, skip below steps
  if (trackFlags(dimension).hasOnlyFixedTracks) {
    return;
  }
...
// https://www.w3.org/TR/css-grid-1/#algo-grow-tracks
void TrackSizing::distributeFreeSpaceToTracks(
    Dimension dimension,
...
void TrackSizing::expandFlexibleTracks(Dimension dimension) {
  if (!trackFlags(dimension).hasFlexibleTracks) {
    return;
  }
  ...
  // max-content contribution.
    ...
    for (auto& track : gridTracks) {
      if (isFlexibleSizingFunction(track.trackSize.maxSizingFunction) &&
          track.trackSize.maxSizingFunction.value().isDefined()) {
        float flexFactor = track.trackSize.maxSizingFunction.value().unwrap();
        if (flexFactor > 1.0f) {
...
float TrackSizing::findFrSize(
  ...
  for (size_t i = startIndex; i < endIndex; i++) {
    ...
    // tracks.
        ...
        track.trackSize.maxSizingFunction.isStretch() &&
        track.trackSize.maxSizingFunction.value().isDefined()) {
      flexFactorSum += track.trackSize.maxSizingFunction.value().unwrap();
      flexibleTracks.push_back(&track);
    }
  }
  ...
  for (auto& track : flexibleTracks) {
    if (track->trackSize.maxSizingFunction.isStretch() &&
        track->trackSize.maxSizingFunction.value().isDefined()) {
      float flexFactor = track->trackSize.maxSizingFunction.value().unwrap();
...
void TrackSizing::stretchAutoTracks(Dimension dimension) {
  // Fast path - if tracks are fixed-sized, skip below steps
  if (trackFlags(dimension).hasOnlyFixedTracks) {
    return;
  }
...
float TrackSizing::calculateFreeSpace(Dimension dimension) {
...
float TrackSizing::minContentContribution(
    const GridItem& item,
    Dimension dimension,
...
float TrackSizing::maxContentContribution(
    const GridItem& item,
    Dimension dimension,
    const ItemConstraint& itemConstraints) {
  auto marginForAxis = item.node->style().computeMarginForAxis(
...
float TrackSizing::minimumContribution(
    const GridItem& item,
    Dimension dimension,
    const ItemConstraint& itemConstraints) {
  auto& tracks = dimension == Dimension::Width ? columnTracks : rowTracks;
...
float TrackSizing::automaticMinimumSize(
    const GridItem& item,
    Dimension dimension,
    const ItemConstraint& itemConstraints,
  ...
  for (size_t i = startIndex; i < endIndex; i++) {
    ...
    // tracks
    if (tracks[i].trackSize.minSizingFunction.isAuto()) {
...
float TrackSizing::contentBasedMinimum(
    const GridItem& item,
    Dimension dimension,
    const ItemConstraint& itemConstraints,
    const std::vector<GridTrack>& tracks,
...
float TrackSizing::limitedMinContentContribution(
    const GridItem& item,
    Dimension dimension,
    const ItemConstraint& itemConstraints) {
  auto& tracks = dimension == Dimension::Width ? columnTracks : rowTracks;
...
float TrackSizing::computeFixedTracksLimit(
    const GridItem& item,
    Dimension dimension,
    const std::vector<GridTrack>& tracks,
    float containerSize) {
  ...
  float limit = 0.0f;
  for (size_t i = startIndex; i < endIndex; i++) {
    if (!isFixedSizingFunction(tracks[i].trackSize.maxSizingFunction, containerSize)) {
      return YGUndefined;
    }
    auto resolved = tracks[i].trackSize.maxSizingFunction.resolve(containerSize);
...
bool TrackSizing::isFixedSizingFunction(
...
bool TrackSizing::isIntrinsicSizingFunction(
    const StyleSizeLength& sizingFunction,
    float referenceLength) {
  return isAutoSizingFunction(sizingFunction, referenceLength);
}

bool TrackSizing::isAutoSizingFunction(
    const StyleSizeLength& sizingFunction,
    float referenceLength) {
...
bool TrackSizing::isFlexibleSizingFunction(
    const StyleSizeLength& sizingFunction) {
  return sizingFunction.isStretch();
}

bool TrackSizing::isPercentageSizingFunction(
    const StyleSizeLength& sizingFunction) {
  return sizingFunction.isPercent();
}

float TrackSizing::getTotalBaseSize(Dimension dimension) {
  auto containerSize = dimension == Dimension::Width ? containerInnerWidth
                                                     : containerInnerHeight;
  const auto& tracks =
...
bool TrackSizing::hasPercentageTracks(Dimension dimension) const {
  return trackFlags(dimension).hasPercentageTracks;
}

TrackSizing::ContentDistribution TrackSizing::calculateContentDistribution(
    Dimension dimension,
    float freeSpace) {
...
// The item's size in the dimension not currently being sized. Undefined when
// no estimator is set, which is what an indefinite cross size means anyway.
float TrackSizing::estimateCrossDimension(const GridItem& item) const {
  return crossDimensionEstimator ? crossDimensionEstimator(item)
                                 : YGUndefined;
}

TrackSizing::ItemConstraint TrackSizing::calculateItemConstraints(
    const GridItem& item,
    Dimension dimension) {
  float containingBlockWidth = YGUndefined;
  float containingBlockHeight = YGUndefined;
  if (dimension == Dimension::Width) {
    containingBlockHeight = estimateCrossDimension(item);
  } else {
    containingBlockWidth = estimateCrossDimension(item);
  }
...
float TrackSizing::calculateEffectiveRowGapForEstimation() {
  ...
  for (auto& track : rowTracks) {
    if (isFixedSizingFunction(
            track.trackSize.maxSizingFunction, containerInnerHeight)) {
      totalTrackSize +=
          track.trackSize.maxSizingFunction.resolve(containerInnerHeight).unwrap();
    } else {
      return rowGap;
    }
  }
...
float TrackSizing::calculateEffectiveGapFromBaseSizes(Dimension dimension) {
  auto containerSize = dimension == Dimension::Width ? containerInnerWidth
                                                     : containerInnerHeight;
  auto gap = node->style().computeGapForDimension(dimension, containerSize);
  const auto& tracks =
...
bool TrackSizing::contributionsChanged(
    Dimension dimension,
...
bool TrackSizing::itemSizeDependsOnIntrinsicTracks(const GridItem& item) const {
  ...
  if (heightStyle.isPercent()) {
    for (size_t i = item.rowStart; i < item.rowEnd && i < rowTracks.size();
         i++) {
      if (isIntrinsicSizingFunction(
              rowTracks[i].trackSize.minSizingFunction, containerInnerHeight) ||
          isIntrinsicSizingFunction(
              rowTracks[i].trackSize.maxSizingFunction, containerInnerHeight)) {
        return true;
      }
    }
  }
...
void TrackSizing::computeItemTrackCrossingFlags() {
  for (auto& item : gridItems) {
    ...
    for (size_t i = item.columnStart; i < item.columnEnd; i++) {
      if (isFlexibleSizingFunction(columnTracks[i].trackSize.maxSizingFunction)) {
        item.crossesFlexibleColumn = true;
      } else if (isIntrinsicSizingFunction(
                     columnTracks[i].trackSize.maxSizingFunction,
                     containerInnerWidth)) {
        item.crossesIntrinsicColumn = true;
      }
      if (isIntrinsicSizingFunction(
              columnTracks[i].trackSize.minSizingFunction, containerInnerWidth)) {
        item.crossesIntrinsicColumn = true;
      }
    }

    for (size_t i = item.rowStart; i < item.rowEnd; i++) {
      if (isFlexibleSizingFunction(rowTracks[i].trackSize.maxSizingFunction)) {
...
TrackSizing::CrossDimensionEstimator
TrackSizing::makeRowHeightEstimatorUsingFixedTracks(float gap) {
  ...
  return [&tracks, containerHeight, gap](const GridItem& item) -> float {
    ...
    for (size_t i = item.rowStart; i < item.rowEnd && i < tracks.size();
         i++) {
      if (isFixedSizingFunction(
              tracks[i].trackSize.maxSizingFunction, containerHeight)) {
        height +=
            tracks[i].trackSize.maxSizingFunction.resolve(containerHeight).unwrap();
        if (i < item.rowEnd - 1) {
          height += gap;
        }
      } else {
...
TrackSizing::CrossDimensionEstimator
TrackSizing::makeCrossDimensionEstimatorUsingBaseSize(
    Dimension dimension,
    float gap) {
